...
test("Windows NSIS packaging is per-user T Books with no updater bundle", () => {
  ...
  assert.doesNotMatch(conf, /downloadBootstrapper/);
  ...
  assert.match(hooks, /embedBootstrapper/);
  assert.match(hooks, /LinkId=2124703/);
  assert.match(hooks, /TBooksWebView2Setup/);
  assert.match(hooks, /does not Abort Setup/i);
  ...
  assert.match(release, /only.*T-Books-Setup\.exe/i);
  assert.match(release, /wizard can have several steps/i);
  assert.match(release, /embeds.*WebView2 bootstrapper/i);
  assert.match(release, /no Node, no Rust/i);
  assert.match(release, /%LOCALAPPDATA%\\T-Books/);
  assert.doesNotMatch(release, /TBooksWebView2Setup/);
  const quickStart = readFileSync(join(root, "docs/QUICK-START.md"), "utf8");
  const readme = readFileSync(join(root, "README.md"), "utf8");
  assert.match(quickStart, /Run \*\*only\*\* `T-Books-Setup\.exe`/);
  assert.match(quickStart, /wizard can have several steps/i);
  assert.match(quickStart, /do \*\*not\*\* install Node, Rust, or WebView2 yourself/i);
  assert.match(quickStart, /%LOCALAPPDATA%\\T-Books/);
  assert.doesNotMatch(quickStart, /TBooksWebView2Setup/);
  assert.match(readme, /run \*\*only\*\* `T-Books-Setup\.exe`/i);
  assert.match(readme, /wizard can have several steps/i);
  assert.match(readme, /do not install Node, Rust, or WebView2 yourself/i);
