...
mod tests {
    ...
    #[test]
    fn submit_posted_forever_refuses() {
        let d = decide(
            &ev(Intent::Submit, "PUR-0001-01"),
            &Facts {
                posted_forever: true,
                ..base()
            },
    ...
    #[test]
    fn refresh_offline_does_not_wipe() {
        let d = decide(&ev(Intent::Refresh, "*"), &Facts { online: false, ..base() });
        match d {
            Decision::Refuse { message } => assert!(message.contains("Offline")),
            other => panic!("{other:?}"),
        }
    ...
    #[test]
    fn refresh_clean_applies() {
        let d = decide(&ev(Intent::Refresh, "*"), &base());
        assert_eq!(d, Decision::ApplyRefresh { skip_keys: vec![] });
    }

    #[test]
    fn zef_appends_never_updates() {
        let books = open_memory().unwrap();
        let e = ev(Intent::Save, "20");
        let d = decide(&e, &base());
        ...
        assert_eq!(rows[0], ("save".into(), "park".into()));
        assert_eq!(rows[1], ("save".into(), "park".into()));
    }

    #[test]
    fn different_keys_have_separate_logs() {
        let books = open_memory().unwrap();
        append_event(books.conn(), &ev(Intent::Save, "20"), &Decision::Park, "").unwrap();
        append_event(books.conn(), &ev(Intent::Save, "21"), &Decision::Park, "").unwrap();
        assert_eq!(list_events_for_key(books.conn(), "voucher", "20").unwrap().len(), 1);
        assert_eq!(list_events_for_key(books.conn(), "voucher", "21").unwrap().len(), 1);
    }
}
